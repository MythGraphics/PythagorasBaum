...
void main() {
    ...
    vec3 diffuse = (diff * diffuseColor) * baseColor * lightIntensity;
    ...
    result = pow( result, vec3( 1.0 / 1.3 )); // Gamma-Korrektur
    FragColor = vec4( clamp( result, 0.0, 1.0 ), 1.0 ); // optional: Begrenzung auf den sichtbaren Bereich
